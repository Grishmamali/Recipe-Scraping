...
public class HypothyroidToAdd extends Baseclass{
		...
		try {
			...
			for(int r=1;r<=totalRows;r++) {
				...
				for(int c=0;c<totalcols;c++){
					...
					// xlsheet Write path 
					String path =".\\sheets\\Hyperthyrod To Add Recipes.xlsx";
						...
						for(int j=1;j<RecipesURLsize;j++) {
							...
							try {
								...
								for (int f=0; f<nameFilter.size(); f++) {
									...
									String name=nameFilter.get(f).toLowerCase();
								...
								for (int a=0; a<RecipeFilter.size(); a++) {
									...
									String Rname=RecipeFilter.get(a).toLowerCase();
