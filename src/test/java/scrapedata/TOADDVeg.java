...
public class TOADDVeg extends Baseclass {
	...
	private List<String> IngredientFilter2() {
		   return Arrays.asList("Beetroot","Blueberries",
				   "Strawberries","Bananas","Avocado",
				   "Tomato","Sweet potato","Mushroom",
				   "Celery","Kiwi","Dark chocolate","Watermelon","Spinach","cabbage","romaine lettuce","mustard greens",
				   "broccoli","argula","fennel","kale","Garlic","Pomegranate","Cinnamon",
				   "Pistachios","Chia seeds","Yogurt","unsalted nuts","chicken","fish","turkey",
	...
	public void recipeTODO () throws Exception {
		...
		String morbidcon = driver.findElement(By.xpath("//a[@id='ctl00_cntleftpanel_ttlhealthtree_tvTtlHealtht167']")).getAttribute("text");
		String MorbidCondition = morbidcon.substring(0,19);
		
		WebElement vegbtn  = driver.findElement(By.xpath("//a[@id='ctl00_cntleftpanel_cattreecuisine_tvCuisinet28']"));
		vegbtn.click();
		...
		String path =".\\sheets\\TOADDveg.xlsx";
				...
				for (int i=1; i<=noofrecipepages; i++){
					...
					for (int j=1; j<recipelinks.size(); j++){
						...
						for (int r=0; r<RecipeFilter.size(); r++) {
							...
							String Rname=RecipeFilter.get(r);
...
//								System.out.println("Nutrient Values:-----"+nutrient);
								...
								xlutils.setCellData("Sheet1", cell, 8, MorbidCondition);
								cell++;
