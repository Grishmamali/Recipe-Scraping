...
public class ToaddDiabetes extends Baseclass {
	...
	public static void DiabeticSearch() throws IOException, InterruptedException {
		...
		String morbodities = driver
				.findElement(By.xpath("//*[@style='font:inherit;display:inline;margin:0px;padding:0px;']")).getText();
		...
		try {
			...
			int totalRows = xlutilsRead.getRowCount("DBToAdd");
			int totalcols = xlutilsRead.getCellCount("DBToAdd", 2);
			for (int r = 1; r <= totalRows; r++) {
				...
				for (int c = 0; c < totalcols; c++) {
					...
					try {
						String searchData = xlutilsRead.getCellData("DBToAdd", r, c);
						...
						// xlsheet Write path
						String path = ".\\sheets\\To_Add_Diabetic.xlsx";
						...
						for (int i = 1; i <= NoOfPages; i++) {
							...
							for (int j = 1; j < RecipesURLsize; j++) {
								...
								try {
									...
									for (Ingredients.DiabetesEliminate eliminate : Ingredients.DiabetesEliminate
											.values()) {
										if (ingredients.toLowerCase().contains(eliminate.getValue().toLowerCase())) {
											System.out.println(
													"Skip Recipe : " + recipeName + " Found " + eliminate.getValue());
											isEliminate = true;
											break;
										}
									}
